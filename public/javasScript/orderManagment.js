document.addEventListener("DOMContentLoaded", () => {
  ...
  function openEditOrderForm(orderRow) {
    ...
    document.getElementById("quantityInput").value =
      orderRow.dataset.quantity || "";
    document.getElementById("totalPriceInput").value =
      parseFloat(
        orderRow.querySelector("td:nth-child(5)").innerText.replace("$", "")
      ) || "";
    ...
    document.getElementById("statusInput").value = statusSpan
      ? statusSpan.innerText.toLowerCase()
      : "";

    document.getElementById("paymentMethodInput").value =
      orderRow.dataset.payment_method || "";
  }
    ...
    fetch(`/orders/${orderId}`, {
      ...
      .then((updatedData) => {
        ...
        row.querySelector("td:nth-child(1)").innerText =
          updatedData.orderId || updatedOrder.orderId;
...
function logout() {
  localStorage.removeItem("token"); // or 'adminToken' if you're using that
